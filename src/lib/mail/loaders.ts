...
export const getProjectEmails = cache(async (projectId: string, page = 0) => {
	"use server";
	...
	const [total, rows] = await Promise.all([
		db.query.emailsTable.findFirst({
			where: and(
				eq(emailsTable.projectId, projectId),
				gt(emailsTable.sentAt, DateTime.now().minus({ days: 30 }).toJSDate()),
			),
			columns: {},
			extras: {
